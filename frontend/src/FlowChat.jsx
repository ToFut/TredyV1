...
const FlowChat = () => {
    ...
    useEffect(() => {
      ...
      const initializeCollaboration = async () => {
        try {
          await collaborationService.initialize('user-1', 'You');
          
          // Set up collaboration message handler
          collaborationService.onCollaborationMessage = (threadId, message) => {
            // Handle real-time collaboration messages
            console.log('Real-time collaboration message:', { threadId, message });
            
            // Update messages if it's a collaboration message
      ...
      // Cleanup collaboration service on unmount
      return () => {
    ...
    const sendMessage = async (text, threadId = null, model = currentModel) => {
      ...
      // Send collaboration message to other users in the thread
      if (threadId) {
        try {
          await collaborationService.sendCollaborationMessage(threadId, {
            type: 'collaboration',
            content: text,
            sender: 'user',
            threadId,
      ...
      if (threadId) {
        ...
        // Map display names to actual model IDs for the backend - OpenAI only
        const modelMapping = {
          'GPT-4': 'gpt-4o',
          'GPT-3.5': 'gpt-3.5-turbo',
          'Claude-3.5': 'gpt-4o', // Using GPT-4o instead
          'Gemini-Pro': 'gpt-4o', // Using GPT-4o instead
          'Base-44-API': 'gpt-4o',
          'Claude-Code': 'gpt-4o', // Using GPT-4o for code generation
          'Gemini-Code': 'gpt-4o', // Using GPT-4o for code generation
          'Sora': 'gpt-4o',
          'Veo': 'gpt-4o', // Using GPT-4o for video content
          'Mistral': 'gpt-4o',
          'LangChain': 'gpt-4o',
        ...
        // Generate real ChatGPT response using streaming API
        const requestData = {
          content: text,
          conversationId: currentSessionId || '22222222-2222-2222-2222-222222222222',
          model: actualModelId,
    ...
    const openThread = (parentMsgId, selectedText = null, parentThreadId = null, preferredModel = null) => {
      console.log('🔍 openThread called with:', { parentMsgId, selectedText, parentThreadId, preferredModel });
      ...
      const newThread = {
        ...
        setThreads(prev => {
          ...
          console.log('🔍 Updated threads count:', updatedThreads.length);
          console.log('🔍 New thread added successfully');
          return updatedThreads;
        });
        
        // Initialize thread draft text
        setThreadDraftText(prev => {
          console.log('🔍 Initializing thread draft text for:', threadId);
      ...
      setTimeout(() => {
        ...
        alert(`Thread created successfully! Thread ID: ${threadId}`);
    ...
    const createSuggestedThread = (parentMsgId, suggestion, parentThreadId = null, preferredModel = null) => {
      ...
      // Don't auto-start with suggested question to prevent loops
      // User can manually send the message if they want
      } catch (error) {
        console.error('Error creating suggested thread:', error);
        // Fallback: just create a simple thread
        const simpleThreadId = generateId();
        const simpleThread = {
          id: simpleThreadId,
          sessionId: currentSessionId,
          parentMsgId,
        ...
        setThreads(prev => [...prev, simpleThread]);
      }
    };
    ...
    const handleTextSelection = (messageId, messageContent) => {
      ...
      if (selectedText.replace(/\s|"/g, '').length > 0) {
        ...
        if (element && selectedText.length > 5) {
    ...
    const navigateToThread = (threadId) => {
      console.log('🔍 navigateToThread called with threadId:', threadId);
      console.log('🔍 Current threads:', threads.map(t => ({ id: t.id, title: t.title })));
      ...
      if (thread) {
        ...
        console.log('✅ Thread navigation completed for:', threadId);
        console.log('🔍 Current view after:', threadId);
      } else {
        console.error('❌ Thread not found:', threadId);
        console.error('❌ Available thread IDs:', threads.map(t => t.id));
      }
    };
    ...
    const buildThreadBreadcrumbs = (thread) => {
      ...
      const label = thread.title || (thread.selectedText ? 
      ...
      crumbs.push({ 
        label, 
        id: thread.id,
        icon: thread.isSuggested ? '💡' : '🧵',
        type: 'thread',
        preview: thread.selectedText?.slice(0, 30) || thread.preview
      });
      return crumbs;
    };
    ...
    const inviteExpert = async (threadId, inviteData) => {
      if (!inviteData) return;
      
      try {
        // Send invitation via collaboration service
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${availableModels.find(m => m.id === summaryModel)?.color?.replace('text-', 'bg-').replace('100', '500') || 'bg-gray-500'}`}></div>
                      <span>{availableModels.find(m => m.id === summaryModel)?.name || summaryModel}</span>
                    </div>
                  ...
                  {availableModels.map(model => (
                        <button
                          key={model.id}
                          onClick={() => {
                            setSummaryModel(model.id);
                            setModelDropdownOpen(false);
                          }}
                          className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-50 flex items-center space-x-2 ${
                            summaryModel === model.id ? 'bg-blue-50 border-l-2 border-blue-500' : ''
                          }`}
                        >
                          <div className={`w-3 h-3 rounded-full ${model.color.replace('text-', 'bg-').replace('100', '500')}`}></div>
                          <div className="flex-1">
    ...
    const CollaborationModal = () => {
      ...
      const handleInvite = async () => {
        ...
        try {
          await inviteExpert(currentInviteThread, inviteData);
        ...
        <div className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
          <div
            className={`max-w-4xl px-6 py-4 rounded-2xl ${
              message.sender === 'user'
                ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-[1.02]'
                : message.type === 'thread-outcome' || message.type === 'chat-summary'
                ? 'bg-gradient-to-r from-purple-50 to-blue-50 text-gray-900 border-2 border-purple-200 shadow-lg hover:shadow-xl'
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                {message.contentType === 'code' ? (
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-purple-50 p-4 rounded-lg border border-pink-200">
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-sm font-medium text-pink-700">Video Generation</span>
                    </div>
                    <div className="bg-black rounded-lg p-4 text-center">
                      <div className="text-white text-sm mb-2">Video Preview</div>
                      <div className="w-full h-32 bg-gradient-to-br from-purple-500 to-pink-500 rounded flex items-center justify-center">
                        <span className="text-white text-2xl">🎬</span>
                      </div>
                      <div className="text-gray-300 text-xs mt-2">AI-generated video content</div>
                    </div>
                    <div className="mt-3 text-sm text-gray-600">{message.content}</div>
                  </div>
                ) : message.contentType === 'audio' ? (
                  <div className="bg-gradient-to-r from-blue-50 to-cyan-50 p-4 rounded-lg border border-blue-200">
                    <div className="flex items-center mb-3">
                      <span className="text-blue-600 mr-2">🎵</span>
                      <span className="text-sm font-medium text-blue-700">Audio Generation</span>
                    </div>
                    <div className="bg-white rounded-lg p-4 border">
                      <div className="flex items-center justify-center space-x-4">
                        <button className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center text-white hover:bg-blue-600">
                          ▶️
                        </button>
                      ...
                      <div className="text-sm text-gray-600 mt-2">{message.content}</div>
                    </div>
                  </div>
                ) : message.contentType === 'visualization' ? (
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
                    <div className="flex items-center mb-3">
                      <span className="text-green-600 mr-2">📊</span>
                      <span className="text-sm font-medium text-green-700">Data Visualization</span>
                    </div>
                    <div className="bg-white rounded-lg p-4 border">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          <tr className="border-t">
                            <td className="px-3 py-2">Data 1</td>
                            <td className="px-3 py-2">Data 2</td>
                            <td className="px-3 py-2">Data 3</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                {selectedText && selectionContext?.messageId === message.id && selectedText.replace(/\s|"/g, '').length > 0 && (
                  <div className="mt-4 p-6 bg-gradient-to-r from-blue-50 via-cyan-50 to-blue-50 border border-blue-200 rounded-xl shadow-lg animate-in slide-in-from-top-2 duration-300">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full flex items-center justify-center">
                          <span className="text-white text-sm">✨</span>
                    </div>
                        <div className="text-sm text-blue-800 font-semibold">Create focused discussion</div>
                      </div>
                      <button
                        onClick={() => {
                          setSelectedText('');
                    ...
                    <div className="text-sm text-blue-700 mb-4 italic bg-white p-3 rounded-lg border-l-4 border-blue-400 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-3">
                      <button
                        ...
                        className="px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-sm rounded-xl hover:from-blue-700 hover:to-cyan-700 transition-all duration-200 hover:scale-105 hover:shadow-lg flex items-center space-x-3 shadow-md font-medium"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white text-sm rounded-lg hover:from-emerald-700 hover:to-green-700 transition-all duration-200 hover:scale-105 flex items-center space-x-2 shadow-md"
                      >
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 text-white text-sm rounded-lg hover:from-yellow-700 hover:to-orange-700 transition-all duration-200 hover:scale-105 flex items-center space-x-2 shadow-md"
                      >
              ...
              {message.sender === 'AI' && (
                <div className="ml-3 flex items-center space-x-1">
                  {/* Simple Create Thread Button for Testing */}
                  <button
                    onClick={() => {
                      console.log('🔍 TEST: Simple thread button clicked for message:', message.id);
                  ...
                  <div className="relative group">
                    <button
                      onClick={() => {
                        console.log('🔍 Main thread button clicked for message:', message.id);
                        onCreateThread(message.id);
                      }}
                      className="p-2 rounded-lg opacity-50 hover:opacity-100 hover:bg-gray-100 transition-all"
                      title="Create Thread"
                    >
                    ...
                    <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                      ...
                      <button
                        onClick={() => {
                          ...
                          console.log('🔍 onCreateThread function:', typeof onCreateThread);
    ...
    const ThreadCard = ({ 
      ...
      const getThreadColor = (thread) => {
        if (thread.isSuggested) return 'from-yellow-50 to-orange-50 border-yellow-200';
        if (thread.isImportantNote) return 'from-red-50 to-pink-50 border-red-200';
        if (thread.type === 'ai-group') return 'from-purple-50 to-indigo-50 border-purple-200';
      ...
      const getThreadPreview = () => {
        if (thread.selectedText) {
          return thread.selectedText.length > 60 
      ...
      // Get thread icon based on type
      const getThreadIcon = () => {
        if (thread.isSuggested) return <Lightbulb className="w-4 h-4 text-yellow-600" />;
        if (thread.isImportantNote) return <Bookmark className="w-4 h-4 text-red-600" />;
        if (thread.type === 'ai-group') return <Users className="w-4 h-4 text-purple-600" />;
        return <GitBranch className="w-4 h-4 text-blue-600" />;
      };

      // Get thread status
      const getThreadStatus = () => {
        if (thread.isResolved) return { text: 'Resolved', color: 'bg-green-100 text-green-700' };
        if (thread.isActive) return { text: 'Active', color: 'bg-blue-100 text-blue-700' };
        return { text: 'Open', color: 'bg-gray-100 text-gray-700' };
      };
      ...
      return (
        <div
          className={`thread-card group relative transition-all duration-200 hover:shadow-md ${
            level > 0 ? 'ml-6' : ''
          } ${thread.highlight ? 'ring-2 ring-blue-400 ring-offset-2' : ''}`}
          onMouseEnter={() => setShowActions(true)}
          onMouseLeave={() => setShowActions(false)}
        >
          {/* Visual Connection Line */}
          {level > 0 && (
            <div className="absolute left-3 top-0 w-px h-full bg-gradient-to-b from-blue-200 to-transparent"></div>
          )}
          
          {/* Thread Node */}
          <div className="absolute left-2.5 top-4 w-3 h-3 bg-blue-400 rounded-full border-2 border-white shadow-sm"></div>
          
          <div 
            className={`bg-gradient-to-r ${threadColor} border rounded-2xl p-6 ml-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-lg cursor-pointer relative hover:ring-2 hover:ring-blue-300 active:scale-[0.98]`}
            onClick={(e) => {
              // Only handle clicks on the main card area, not on buttons or interactive elements
              if (e.target.closest('button') || e.target.closest('input') || e.target.closest('a')) {
                return;
              }
              console.log('🔍 ThreadCard clicked for thread:', thread.id);
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center space-x-2">
                <span className={`text-xs px-2 py-1 rounded-full ${threadStatus.color}`}>
                  {threadStatus.text}
                    </span>
            ...
            {showActions && (
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    console.log('🔍 Open button clicked for thread:', thread.id);
                    onNavigateToThread(thread.id);
                  }}
                ...
                <button
                      ...
                      className="text-xs bg-gray-100 text-gray-700 px-3 py-1.5 rounded-lg hover:bg-gray-200 transition-colors flex items-center space-x-1"
                >
                      {isCollapsed ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
                      <span>{childThreads.length} sub</span>
                </button>
                  )}
              ...
              <div className="mt-3 pt-3 border-t border-gray-200 space-y-2">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-white p-2 rounded">
                    ...
                    <span className="ml-1 font-medium">{thread.model || 'GPT-4'}</span>
                </div>
                  <div className="bg-white p-2 rounded">
                    <span className="text-gray-500">Level:</span>
                    <span className="ml-1 font-medium">L{level}</span>
              </div>
                  <div className="bg-white p-2 rounded">
                    <span className="text-gray-500">Created:</span>
                    <span className="ml-1 font-medium">{new Date(thread.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="bg-white p-2 rounded">
                    <span className="text-gray-500">Last Active:</span>
                    <span className="ml-1 font-medium">{new Date(thread.lastActiveAt).toLocaleDateString()}</span>
                  </div>
                </div>
                ...
                {thread.selectedText && (
                  <div className="bg-white p-2 rounded text-xs">
                    ...
                    <p className="mt-1 text-gray-700 italic">"{thread.selectedText}"</p>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Child Threads */}
          {!isCollapsed && childThreads.length > 0 && (
    ...
    const ThreadView = ({ 
      ...
      activeUsers,
      openThread,
      loadThreadCollaborators,
      loadThreadActiveUsers
    }) => {
      console.log('🔍 ThreadView rendered with:', { threadId, onSendMessage: typeof onSendMessage, currentThread: threads.find(t => t.id === threadId) });
      ...
      // Load collaborators and active users when thread opens
      useEffect(() => {
        if (threadId && loadThreadCollaborators && loadThreadActiveUsers) {
          loadThreadCollaborators(threadId);
          loadThreadActiveUsers(threadId);
          
          // Update presence for this thread
          collaborationService.updatePresence(threadId);
        }
      }, [threadId, loadThreadCollaborators, loadThreadActiveUsers]);
      ...
      return (
        <div className="flex-1 flex flex-col h-full">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    ...
                    <div className="px-3 py-2 border-b border-gray-100">
                      ...
                      <div className="relative">
                        <button
                          onClick={() => setThreadModelDropdownOpen(!threadModelDropdownOpen)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm text-left flex items-center justify-between hover:bg-gray-50"
                        >
                          <div className="flex items-center space-x-2">
                            <div className={`w-3 h-3 rounded-full ${availableModels.find(m => m.id === currentThread.model)?.color?.replace('text-', 'bg-').replace('100', '500') || 'bg-gray-500'}`}></div>
                            <span>{availableModels.find(m => m.id === currentThread.model)?.name || currentThread.model}</span>
                          </div>
                          <ChevronDown className="w-4 h-4 text-gray-400" />
                        </button>
                        
                        {threadModelDropdownOpen && (
                          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto">
                        {availableModels.map(model => (
                              <button
                                key={model.id}
                                onClick={() => {
                                  onSwitchModel(currentThread.id, model.id);
                                  setThreadModelDropdownOpen(false);
                                }}
                                className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-50 flex items-center space-x-2 ${
                                  currentThread.model === model.id ? 'bg-blue-50 border-l-2 border-blue-500' : ''
                                }`}
                              >
                                <div className={`w-3 h-3 rounded-full ${model.color.replace('text-', 'bg-').replace('100', '500')}`}></div>
                                <div className="flex-1">
            ...
            {threadMessages.map((msg, index) => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className={`text-sm leading-relaxed ${msg.sender === 'user' ? 'text-white' : 'text-gray-900'}`}>{msg.content}</p>
                ...
                <div className="space-y-3">
                  {suggestedQuestions.map((question, index) => (
                    <button
                      ...
                      className="w-full text-left text-sm p-4 bg-white border border-yellow-300 rounded-xl hover:bg-yellow-100 hover:border-yellow-400 transition-all duration-200 hover:scale-[1.02] hover:shadow-md text-gray-900 font-medium"
                    >
                      {question}
                    </button>
                  ))}
          ...
          {/* Thread Input */}
          <div className="p-6 border-t border-gray-200/60 bg-white/80 backdrop-blur-sm flex-shrink-0">
            <div className="flex space-x-4">
              {/* Test button to check if input is working */}
              <button
                onClick={() => {
                  console.log('🔍 Test button clicked - setting test text');
                  setThreadDraftText(prev => ({ ...prev, [threadId]: 'Test message from button' }));
                }}
              ...
              <input
                type="text"
                value={threadDraftText[threadId] || ''}
                onChange={(e) => {
                  console.log('🔍 Thread input onChange:', { threadId, value: e.target.value, currentThreadId: currentThread.id });
                  setThreadDraftText(prev => ({ ...prev, [threadId]: e.target.value }));
                }}
                onFocus={(e) => {
                  console.log('🔍 Thread input onFocus:', { threadId, currentThreadId: currentThread.id });
                }}
                onBlur={(e) => {
                  console.log('🔍 Thread input onBlur:', { threadId, currentThreadId: currentThread.id });
                }}
                placeholder={`Continue ${currentThread.selectedText ? 'focused' : 'thread'} discussion...`}
                className="flex-1 px-6 py-4 border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400 shadow-sm hover:shadow-md transition-shadow duration-200"
                onKeyPress={(e) => {
                  console.log('🔍 Thread input onKeyPress:', { key: e.key, threadId, currentThreadId: currentThread.id, hasText: !!threadDraftText[threadId]?.trim() });
                  if (e.key === 'Enter' && threadDraftText[threadId]?.trim()) {
                    console.log('🔍 Sending thread message:', threadDraftText[threadId]);
                    onSendMessage(threadDraftText[threadId], currentThread.id, currentThread.model);
                    setThreadDraftText(prev => ({ ...prev, [threadId]: '' }));
                  }
                }}
              />
              <button
                onClick={() => {
                  console.log('🔍 Send button clicked:', { threadId, currentThreadId: currentThread.id, hasText: !!threadDraftText[threadId]?.trim(), text: threadDraftText[threadId] });
                  if (threadDraftText[threadId]?.trim()) {
                    console.log('🔍 Sending thread message via button:', threadDraftText[threadId]);
                    onSendMessage(threadDraftText[threadId], currentThread.id, currentThread.model);
                    setThreadDraftText(prev => ({ ...prev, [threadId]: '' }));
                  }
                }}
                disabled={!threadDraftText[threadId]?.trim()}
                className="px-6 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:scale-105 hover:shadow-lg flex items-center space-x-2 shadow-md"
              >
    ...
    // Load thread collaborators
    const loadThreadCollaborators = async (threadId) => {
      try {
        const collaborators = await collaborationService.getThreadCollaborators(threadId);
        setCollaborators(prev => {
          const newCollaborators = [...prev];
          collaborators.forEach(collaborator => {
            const existingIndex = newCollaborators.findIndex(c => c.id === collaborator.id);
            if (existingIndex >= 0) {
              newCollaborators[existingIndex] = collaborator;
            } else {
        ...
        console.error('Failed to load thread collaborators:', error);
      }
    };

    // Load active users for a thread
    const loadThreadActiveUsers = async (threadId) => {
      try {
        const activeUsers = await collaborationService.getActiveUsers(threadId);
        setActiveUsers(prev => {
          const newActiveUsers = [...prev];
          activeUsers.forEach(user => {
            const existingIndex = newActiveUsers.findIndex(u => u.id === user.userId);
            if (existingIndex >= 0) {
              newActiveUsers[existingIndex] = { ...newActiveUsers[existingIndex], ...user };
            } else {
              newActiveUsers.push({
                id: user.userId,
                name: user.userName,
    ...
    return (
      <div className="h-screen flex flex-col bg-gradient-to-br from-gray-50 via-white to-blue-50">
        <OnboardingTour />
        <CollaborationModal />
        
        {/* Enhanced Header */}
        <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200/60 px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-10">
          <div className="flex items-center space-x-4" id="navigation">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2 hover:bg-gray-100 rounded-xl transition-all duration-200 hover:scale-105 lg:hidden"
            >
                  ...
                  <button
                    ...
                    className={`font-medium transition-all duration-200 hover:scale-105 ${
                      index === breadcrumbs.length - 1 
                        ? 'text-gray-900 bg-gray-100 px-3 py-1 rounded-lg' 
                        : 'text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-2 py-1 rounded-lg'
                    }`}
                  >
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="relative group">
              <button 
                className="p-2 text-gray-600 hover:bg-gray-100 rounded-xl transition-all duration-200 hover:scale-105 hover:shadow-md"
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <main className="flex-1 flex flex-col">
            {!messages.filter(m => m.sessionId === currentSessionId && !m.threadId).length > 0 && currentView === 'main' ? (
              <div className="flex-1 flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
                <div className="text-center mb-12">
                  ...
                  <p className="text-lg text-gray-600 max-w-md mx-auto">
                    Start a conversation and explore ideas with AI-powered threading
                  </p>
                </div>
                ...
                <div className="w-full max-w-3xl flex flex-col items-center space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full mb-8">
                    {[
                      { icon: '💡', text: 'Ask a question', color: 'from-blue-500 to-cyan-500' },
                      { icon: '🧠', text: 'Explore ideas', color: 'from-purple-500 to-pink-500' },
                      { icon: '🚀', text: 'Get creative', color: 'from-green-500 to-emerald-500' }
                    ].map((suggestion, index) => (
                      <button
                        key={index}
                        onClick={() => setDraftText(suggestion.text)}
                        className="group p-6 bg-white rounded-2xl shadow-sm border border-gray-200 hover:shadow-lg transition-all duration-300 hover:scale-105 hover:border-gray-300"
                      >
                        <div className={`w-12 h-12 bg-gradient-to-r ${suggestion.color} rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-200`}>
                  ...
                  <div className="flex items-center w-full bg-white border border-gray-200 rounded-2xl shadow-lg px-6 py-5 hover:shadow-xl transition-shadow duration-300">
                    <input
                      type="text"
                      value={draftText}
                    ...
                    <button
                      ...
                      className="ml-3 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 transition-all duration-200 hover:scale-105 hover:shadow-lg flex items-center space-x-2"
                    >
                      <Send className="w-5 h-5" />
                      <span className="font-medium">Send</span>
                    </button>
              ...
              <>
                <div ref={chatRef} className="flex-1 overflow-y-auto p-6 space-y-6" id="main-chat">
                  {messages
                ...
                <footer className="bg-white border-t border-gray-200 p-4">
                  <div className="flex items-center space-x-3 mb-3">
                    ...
                    <div className="flex-1 relative">
                      <input
                        ...
                        className="w-full px-6 py-4 border border-gray-300 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 placeholder-gray-400 shadow-sm hover:shadow-md transition-all duration-200 pr-16"
                      />
                      ...
                      <button
                        ...
                        disabled={!draftText.trim()}
                          className="p-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl hover:from-blue-700 hover:to-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:scale-105 hover:shadow-lg"
                      >
                  ...
                  {/* Quick Actions Panel */}
                  {showQuickActions && (
                    <div className="mt-4 p-4 bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 shadow-lg animate-in slide-in-from-top-2 duration-300">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                          { icon: '💡', label: 'Ask Question', action: () => setDraftText('Can you help me with...') },
                          { icon: '🧠', label: 'Brainstorm', action: () => setDraftText('Let\'s brainstorm about...') },
                          { icon: '📝', label: 'Summarize', action: () => setDraftText('Please summarize...') },
                          { icon: '🔍', label: 'Research', action: () => setDraftText('Research about...') }
                        ].map((action, index) => (
                          <button
                            key={index}
                            onClick={() => {
                            ...
                            className="p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 hover:shadow-md transition-all duration-200 hover:scale-105 flex flex-col items-center space-y-2"
                          >
              ...
              <ThreadView 
                ...
                loadThreadCollaborators={loadThreadCollaborators}
                loadThreadActiveUsers={loadThreadActiveUsers}
              />
            )}
          </main>
        </div>
